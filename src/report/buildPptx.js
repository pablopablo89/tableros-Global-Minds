...
export async function generarPptx(data, cfg, seleccion, periodo = null) {
  ...
  if (inc('prog_diplomados') && segDiplomados(cfg)) slidePrograma(pptx, `Detalle por programa — ${segDiplomados(cfg).nombre}`, consolidarProgramas(programasDe(data, segDiplomados(cfg).id)).filter((p) => p.total >= 3), acc)
